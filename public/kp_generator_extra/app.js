...
class CPGenerator {
    ...
    shouldShowImplementationSection() {
        ...
        if (this.state.editOfferId) {
            const hasExtras = Array.isArray(this.state.customOneTimePayments)
                && this.state.customOneTimePayments.some((p) => (Number(p?.price) || 0) > 0 || String(p?.name || '').trim() !== '');
            const hasDiscount = (Number(this.state.implementationDiscountPercent) || 0) > 0
                || (Number(this.state.implementationDiscountPercent12) || 0) > 0;
                ...
                <div class="service-header">
                    <div class="service-info">
                        ...
                        <div class="service-price" style="margin-top: 6px; font-size: 0.9rem; color: #111;">
                            ${(isIncluded && !isConnectionExtraServices) && !service.hasChannels ? 'Включено' : `${this.formatServicePrice(unitPrice)}${service.hasChannels ? ' /канал/мес' : ' /мес'}`}
                        </div>
