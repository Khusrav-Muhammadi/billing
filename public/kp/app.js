    ...
    bindEvents() {
        ...
        if (clientSelect) {
            clientSelect.addEventListener('change', (e) => {
                ...
                // Применяем персональные цены если есть
                const clientPrices = window.CP_CLIENT_PRICES?.[clientId];
                if (clientPrices) {
                    // Перезаписываем цены тарифов
                    if (clientPrices.tariffs) {
                        Object.entries(clientPrices.tariffs).forEach(([key, prices]) => {
                            if (this.config.tariffs[key]) {
                                this.config.tariffs[key].prices = {
                                    ...this.config.tariffs[key].prices,
                                    ...prices
                                };
                            }
                    ...
                    // Перезаписываем цены услуг
                    if (clientPrices.services) {
                        Object.entries(clientPrices.services).forEach(([key, prices]) => {
                            if (this.config.services[key]) {
                                this.config.services[key].prices = {
                                    ...this.config.services[key].prices,
                                    ...prices
                                };
                            }
                    ...
                    // Если у клиента нет персональных цен — восстанавливаем базовые
                    this.config = JSON.parse(JSON.stringify(window.CP_CONFIG));
                }

                // Автопереключение валюты
