...
function isValidPhone(string $phone, int $organization_id): ?string
{
    ...
    if ($validator->fails()) {
        $localization = Setting::where('organization_id', $organization_id)->value('localization');

        if (!$localization) {
            return null;
        }

        $phoneWithCountryCode = '+' . ltrim($localization, '+') . ltrim($phone, '+');
        ...
        $phone = $phoneWithCountryCode;
        \Illuminate\Support\Facades\Log::error($phone);
        $phone = new PhoneNumber($phone);
        return ltrim($phone->formatE164(), '+');
    }
